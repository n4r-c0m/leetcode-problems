...
class Solution {
    ...
    function findAllPeople($n, $meetings, $firstPerson) {
        $processedPersons = [0];
        $persons = array_fill(1, $n - 2, []);
        $persons[0][0] = [$firstPerson];
        foreach ($meetings as $meeting) {
            $persons[$meeting[0]][$meeting[2]][] = $meeting[1];
            $persons[$meeting[1]][$meeting[2]][] = $meeting[0];
        }

        $earliest[0] = 0;

        $nextPersons = new SplMinHeap();
        $nextPersons->insert([0, 0]);
        while (!$nextPersons->isEmpty()) {
            $personId = $nextPersons->extract()[1];

            $processedPersons[$personId] = true;
            foreach ($persons[$personId] ?? [] as $meetingTime => $meetingPersons) {
                if ($meetingTime < $earliest[$personId]) {
                    continue;
                }

                foreach ($meetingPersons as $meetingPerson) {
                    if ($meetingTime > (@$earliest[$meetingPerson] ?? PHP_INT_MAX)) {
                        continue;
                    }

                    $earliest[$meetingPerson] = min($meetingTime, @$earliest[$meetingPerson] ?? PHP_INT_MAX);
                    $nextPersons->insert([$meetingTime, $meetingPerson]);
                }
